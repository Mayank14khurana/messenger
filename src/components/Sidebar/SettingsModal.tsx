...
const SettingsModal: React.FC<SettingsModalProps> = ({ currentUser, onClose, isOpen }) => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = (data) => {
    setIsLoading(true);
    axios.post('/api/settings', data).then(() => { router.refresh(); onClose() }).catch(() => toast('Something went wrong')).finally(() => setIsLoading(false));
  }
  return (
            ...
            <div className="mt-10 flex flex-col gap-y-8">
              ...
              <div>
                ...
                <div className="flex mt-2 items-center gap-x-3">
                 <Image alt="User image" src={image ||currentUser?.image || '/placeholders.jpeg'} height={48} width={48} className="rounded-full" />
                 <CldUploadButton options={{maxFiles:1}} onSuccess={handleUpload} uploadPreset="m6eldilq">
